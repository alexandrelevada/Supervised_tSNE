...
Supervised Entropic SNE and t-SNE implementation
...
"""Compute matrix containing negative KL-divergences
   for all pairs of patches in input matrix X using
   KNN graph
    ...
    """
def neg_entropic_dists(X, y):
    # Number of samples
    n = X.shape[0]
    m = X.shape[1]
    k = round(np.sqrt(n))
    # Generate KNN graph
    knnGraph = sknn.kneighbors_graph(X, n_neighbors=k, mode='connectivity', include_self=False)
    A = knnGraph.toarray()
    # Inicial matrices for storing the means and covariance matrices for each patch
    medias = np.zeros((n, m))
    matriz_covariancias = np.zeros((n, m, m))
    # Extract patches from KNN graph
    for i in range(n):       
        vizinhos = A[i, :]
    ...
    # Computes the KL-divergence matrix
    D = A.copy()
    for i in range(D.shape[0]):
        for j in range(D.shape[1]):
            euclidean = norm(X[i, :] - X[j, :])**2
            ent = divergenciaKL(medias[i, :], medias[j, :], matriz_covariancias[i, :, :], matriz_covariancias[j, :, :])         # KL   
            #ent = Bhattacharyya(medias[i, :], medias[j, :], matriz_covariancias[i, :, :], matriz_covariancias[j, :, :])        # Bhattacharyya    
            #ent = CauchySchwarz(medias[i, :], medias[j, :], matriz_covariancias[i, :, :], matriz_covariancias[j, :, :])        # Cauchy-Schwarz    
            if A[i, j] > 0:
                if y[i] == y[j]:
                    D[i, j] = min(euclidean, ent)    
                else:
                    D[i, j] = euclidean + ent
            else:
                D[i, j] = euclidean
...
def q_joint(Y, y):
    ...
    distances = neg_entropic_dists(Y, y)
...
def p_joint(X, y, target_perplexity):
    ...
    distances = neg_entropic_dists(X, y)
